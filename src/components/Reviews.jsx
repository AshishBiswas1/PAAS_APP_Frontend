    ...
    <section id="reviews" className="py-20 bg-neutral-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-neutral-600">Real stories from engineers shipping reliable APIs.</p>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((r, i) => (
            <figure key={i} className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-card">
              <blockquote className="text-neutral-800">“{r.text}”</blockquote>
              <figcaption className="mt-4 text-sm text-neutral-600">{r.name} • {r.role}</figcaption>
            </figure>
          ))}
        </div>
      </div>
