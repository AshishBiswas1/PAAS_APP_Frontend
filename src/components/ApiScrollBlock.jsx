...
const tabs = {
  curl: `curl -s https://api.claritypi.dev/v1/welcome \\
  -H "Authorization: Bearer $TOKEN"`,
  fetch: `await fetch("https://api.claritypi.dev/v1/welcome", { headers: { Authorization: "Bearer " + token } }).then(r => r.json())`,
  axios: `const res = await axios.get("/v1/welcome", { baseURL: "https://api.claritypi.dev", headers: { Authorization: "Bearer " + token } })`
};
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-start">
        <div>
          ...
          <p className="mt-3 text-neutral-600 dark:text-neutral-300">
            A sticky response window creates focus while you scroll the
            page—perfect for explaining what a great API feels like.
          </p>
              ...
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-md ${
                  tab === k
                    ...
                    : 'text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-slate-800/60'
                }`}
              >
                {k.toUpperCase()}
              </button>
          ...
          <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-card bg-white dark:bg-slate-900 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-slate-800/40">
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center px-2 py-0.5 text-xs rounded bg-emerald-100 text-emerald-700">
                  GET
                </span>
